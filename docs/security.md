...
The Node process holds the integration credentials and calls tools directly. There is no internal HTTP tool callback, Python sidecar, shell execution, self-deployment or authenticated browser tool. Gmail and Calendar are read-only. Gmail spends at most 40 API requests per turn, counting every list, metadata and body fetch; the ceiling is enforced in the adapter and returns a non-retryable validation error, so a search loop cannot walk the mailbox even if the model misbehaves. Message bodies are never stored outside the turn that fetched them. Role deletion and skill activation require an exact owner approval; the model cannot approve its own proposal.
