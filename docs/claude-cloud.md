...
2. Configure the cloud environment with Node 22 and npm ci. Permit necessary package-registry and GitHub access. Do not add production bot, model, Google or SSH secrets. The only production-related secret a Claude cloud environment should hold is the read-only log reader for `chief-log-reader-cloud`: `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY` and `AWS_REGION=ap-southeast-1`. Set them in the environment's private settings so `npm run logs:cloudwatch` works; see [lightsail.md](lightsail.md#reading-logs).
