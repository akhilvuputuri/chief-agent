...
Gmail operations are read tools gated by the `gmail` availability flag. `gmail_search(query, pageToken?)` validates a query of at most 500 characters and returns triage metadata per hit; `gmail_thread(threadId)` and `gmail_read(messageId)` accept only hexadecimal identifiers, matching Gmail's own format, so a path-like value is rejected before any Gmail message request is made; the credential refresh and mailbox-identity check may already have run. All three are journalled as reads. Identifiers echoed inside a search response are validated again before they reach a URL, and a row keeps the identifiers that were requested rather than those echoed back. The adapter charges every underlying Gmail message request to the turn's run identifier and refuses beyond 40 with a non-retryable validation error, so the ceiling holds without depending on model judgement (see [Google integrations](google-integrations.md)).
