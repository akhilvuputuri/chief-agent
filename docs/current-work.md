# Chief — last verified release v0.3.22

The repo-controlled main-model policy is a candidate change on the documentation/model branch. Its initial `main: null` preserves the current environment-derived model. Future agents can pin a verified OpenRouter model in a reviewed PR and let the ordinary release deploy it; this is **not** a claim that the live bot has switched models. See [model deployment](deployment.md#changing-the-production-model-through-a-release) and [journal 32](journey/32-repo-controlled-model.md).
