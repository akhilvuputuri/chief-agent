...
Candidate [PR #66](https://github.com/akhilvuputuri/companion-agent/pull/66) implements [issue #64](https://github.com/akhilvuputuri/companion-agent/issues/64), on-demand parcel persistence and selected-email extraction. [Contract](parcels.md), [migration 019 rollout](parcel-rollout.md), [journal](journey/25-delivery-tracker.md). Local check/build/format passed on 20 September; independent review and operator rollout remain pending. UI acceptance is blocked by unavailable test Telegram/runtime/Gmail access. No unsolicited email polling or carrier integration.
