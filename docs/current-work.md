...
Released [v0.3.11](https://github.com/akhilvuputuri/companion-agent/releases/tag/v0.3.11) at `9c71f61d3578ae5718804b56aa281ee469d38396` ([PR #51](https://github.com/akhilvuputuri/companion-agent/pull/51)): NLB catalogue availability with the Lucky Day verdict rule, a pinned-route paced client and a CI boundary test; app-only. Owner acceptance from the phone is pending. Candidate v0.3.12 on `feature/library-account`: migration 016, encrypted Libby identity, phone linking ceremony, `/library` commands and `library_shelf`; operator-released via `scripts/deploy-library.py` from baseline `794da5f` ([rollout](library-rollout.md)). Then approved writes (Phase 3) and the hold-ready watcher (Phase 4). Plan: [issue #41](https://github.com/akhilvuputuri/companion-agent/issues/41); behaviour: [library](library.md); journal: [22](journey/22-library-assistant.md).
