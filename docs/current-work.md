...
Candidate v0.3.11 on `feature/library-catalogue`: NLB catalogue availability (`library_check`, `library_availability`) with the Lucky Day verdict rule, a pinned-route paced client and a CI boundary test. App-only; no migration or Compose change. See [library](library.md), [journal 22](journey/22-library-assistant.md) and the plan in [issue #41](https://github.com/akhilvuputuri/companion-agent/issues/41). Next: independent review of the PR head, merge, release, owner acceptance from the phone, then the operator-released Phase 2 (schema, encrypted identity, linking, shelf).
