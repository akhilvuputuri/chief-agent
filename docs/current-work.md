...
Candidate v0.3.19 on `feature/delivery-tracker` ([issue #64](https://github.com/akhilvuputuri/companion-agent/issues/64)): a delivery tracker with migration 019, append-only parcel history, explicit precedence between owner statements and email, and host-computed matching that asks rather than guesses. Needs the reviewed `scripts/deploy-parcels.py` rollout, which must follow the watchlist rollout because it requires migration 018 to be live first. While building it the fixed model prompt measured 50,156 characters with every capability on, already above the 48,000 soft allowance, so the tool surface was reduced and the capability gated; see [deliveries](deliveries.md) and [journal 26](journey/26-delivery-tracker.md). No owner acceptance yet.
