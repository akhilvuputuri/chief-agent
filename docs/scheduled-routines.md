...
The stock watchlist ([issue #46](https://github.com/akhilvuputuri/chief-agent/issues/46)) landed separately as a deterministic poller, not a routine: an LLM routine could not meet its no-model-per-poll and conditional-silence requirements. See [stock-watchlist.md](stock-watchlist.md). The daily reading bulletin ([issue #50](https://github.com/akhilvuputuri/chief-agent/issues/50)) follows the same deterministic pattern for the same reasons plus per-item feedback buttons; see [reading-bulletin.md](reading-bulletin.md).
