...
- `/roles`, `/items`, `/schedules`, `/drafts`: six-record pages, active/all filter, default/newest sort, detail and Back. Details read current saved data; they do not re-run analysis. `/drafts` has no approval action: use the original Telegram approval card.
