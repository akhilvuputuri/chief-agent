...
The three operations are gated on `availability.parcels`, enabled in the deployment and off by default. This is not an external dependency; it exists because the fixed model prompt is a shared budget. Measured on 23 September 2026 with every other capability on, the fixed prompt is 50,971 characters on the deployed `c936d7d` and 54,244 on this branch, a rise of 3,273, against a 48,000-character soft allowance; 120,000 is the compaction threshold and 400,000 the hard limit. Gating spares minimal and specialist runtimes the 2,658 characters of tool schema and description. The remaining 614, a paragraph of runtime guidance, are paid whether or not the capability is on. An earlier measurement on a different base gave 3,137 in total and 479 always paid; the difference is the observation-time rule added in the third review round. Merging create and update into one `parcel_record`, and folding the single read into `parcel_list(id)`, was done for the same reason. See [journal 31](journey/31-delivery-tracker.md).
