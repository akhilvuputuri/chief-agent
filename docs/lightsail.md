...
2. `chief-log-export.service` polls the journal for `CONTAINER_NAME=hermes-companion-gateway-1`. It keeps only lines starting with `{"schema":"chief.ops/1",` and appends them to `/var/log/chief/gateway.jsonl`. Anything else the container prints stays in the local journal. Delivery is at least once:
   - The exporter polls the journal every 5 seconds. The saved cursor advances only after a pass's lines were written, so a failed write (for example a full disk) is retried rather than skipped. Twelve consecutive failed passes make the service exit, so systemd shows restarts instead of a silent stall. A restart, reboot or crash resumes after that cursor. It does not use `journalctl --follow`, because on systemd 255 `--follow` with `--cursor-file` skips entries from the previous boot. An outage therefore delays lines rather than dropping them, up to the shorter of journal retention and CloudWatch's 14-day limit for event age. Older lines are rejected by CloudWatch but remain in the local journal.
   - After a crash during a pass, that pass's lines can be exported again. On the very first start the retained gateway journal is exported.
...
| `chief-log-reader-local`, `chief-log-reader-cloud` (IAM users) | the local operator profile, and the Claude Code cloud environment's AWS SigV4 API credential | `StartQuery` on the two groups; `GetQueryResults` (AWS supports no resource scoping for this action) | publish, other log groups, deploy, SSH, database    |
...
In Claude Code cloud sessions, the `chief-log-reader-cloud` key is stored as an **API credential** on the cloud environment (claude.ai/code → environment → Edit → API credentials): type **AWS SigV4**, allowed website `logs.ap-southeast-1.amazonaws.com`. The session never sees the key; Claude's proxy signs matching requests. Environment variables are visible to anyone using the environment, so they hold only placeholders: `AWS_ACCESS_KEY_ID=proxy-signed`, `AWS_SECRET_ACCESS_KEY=proxy-signed` and `AWS_REGION=ap-southeast-1`. The SDK needs these to send a request, and the proxy re-signs it. The CLI sends through the proxy named in `HTTPS_PROXY` when one is set, because Node's AWS SDK ignores that variable. Never put the real key in environment variables, a prompt, an issue or a repository file. The CLI uses `ap-southeast-1` and only the two Chief groups. It defaults to one hour and allows at most 24 hours and 500 rows per query. It exits non-zero on access or query failure. Times are given as relative values (`30m`, `2h`) or ISO with an explicit zone. Output shows the window in UTC and Singapore time (UTC+8).
