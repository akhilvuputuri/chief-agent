...
## Fifth real link and the clone recovery — 20 September 2026 (measured)

On v0.3.17 the fourth poll answered `fulfilled` with a `blessing`, then `POST chip/clone {blessing}` again answered 403 with `missing_chip`. Reproduced independently from Libby's own origin with two freshly minted anonymous chips: display code, enter code, poll `fulfilled` + blessing, `POST chip/clone` → 403 `missing_chip`. So the refusal is not about how the chip is minted (v0.3.17 mints exactly as the client does) and not about the card; it is the clone step against a just-minted chip. Libby's client already handles this: every chip-authenticated request runs through a wrapper (`_requestWithChip`) that treats `missing_chip` as recoverable by re-minting the chip once (same identity, `v=<chip prefix>`) and retrying the same request. The assistant treated the 403 as fatal, leaving the attempt `completing` and the approval `uncertain`. Fix: on a `missing_chip`/403 from `POST chip/clone`, re-mint the linking identity once and retry the clone with the re-minted bearer before syncing. Hypothesis under test on the next link: the re-mint registers the chip so the retried clone lands the card.

## Sixth real link, and what the clone still lacks — 21 September 2026 (measured)

On `25aa0e3` the re-mint recovery ran exactly as designed and was still refused: first `POST chip/clone` 403 `missing_chip`, then `POST /chip` 200, then the retried clone 403 again. Re-minting is therefore **not** the missing piece, and the hypothesis from the fifth attempt is disproven.

Two further hypotheses were disproven the same evening. The owner's Libby menu on an established device offers only Back Up Your Data, Copy To Another Device, Diagnostics, Privacy and Reset Everything: there is no recover entry, so the pointer-displays/primary-enters pairing the assistant uses is Libby's real pairing and the earlier role-mismatch theory is wrong.

What remains is a request-shape difference. Libby builds every sentry request with `credentials: true`, so the session established when its chip is minted travels on the poll, the clone and the sync. The assistant's client sent no cookies at all, and the independent browser reproduction that also failed with `missing_chip` likewise omitted credentials, which is consistent with every observed failure. This release carries one `CookieJar` through a ceremony's mint, polls, clone and sync, and drops the `chip/sync` probe that ran between fulfilment and the clone so the order matches the client exactly.

Because two hypotheses in a row were wrong, this release also adds attribution rather than relying on the next guess: a refused clone journals `library.clone_refused` with the response header names, the cookie names held, and a bounded scrubbed excerpt of the error body. Cookie and token values are never recorded, covered by a leak-scan test. If the clone is refused again, the recorded body should say why without spending another attempt.
