...
## Round five: one flag answering two questions

Round five (Claude Fable 5.1, head `bea6672`) confirmed every round-four fix, confirmed the rollout would be accepted against the server's current release, and found one major. The echo rule from round four had been implemented by making an echoing email's status "not applied", and the details rule asked whether an email's status applied. So a newer email that happened to repeat the owner's status wording could not correct the carrier, while an identical email with different wording could. The reviewer also showed the boundary was untested in either direction: the intended gate and the wrong one both passed.

The fix separates the two questions. Is this report current enough to count? That governs details. Does its status change anything? An echo is current but changes nothing. There is now a test on each side of the boundary, and the old gate fails it. The same review noted that the duplicate-tracking check was a read before the write with no database constraint, so two concurrent writes could both pass; the index is now unique and the violation surfaces as the same message.

## Round six: a different reviewer model, and a clock that stopped at the owner

Round six ran on Claude Opus 5.5 (`claude-opus-5-5`) because the account reached its Fable usage limit partway through; the earlier rounds used Claude Fable 5.1. It confirmed the round-five fix and the race fix, confirmed the rollout would be accepted against the live release, and found three majors.

The echo rule kept the owner as the source but never moved the status clock, so the clock stayed at the owner's moment. Gmail lists newest first, so a newer confirming email is often processed before an older one, and the older one was then judged against the owner's earlier moment and won. The same gap let an older echo overwrite a carrier a newer echo had set. A current echo now corroborates: it records the moment the owner's status was last known to hold, emails are judged against that, and the owner is still judged only against their own moment so a corroborating email can never block a correction. A new status discards earlier corroboration.

For `unknown`, the wording is the whole content, so "awaiting collection" and "returned to sender" are different reports; treating the second as an echo discarded it with "you already told me this". Different wording for `unknown` is now new information.
