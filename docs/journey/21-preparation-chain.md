...
Focused synthetic tests cover source/owner isolation, frozen snapshots after live edits, unknown questions, invalid/missing references, multi-role persistence, progress preservation, bounded reads and Sheet cells. Local typecheck/build and all 240 application tests plus two Google-scope tests passed. Twelve offline operator-rollout checks passed. The first focused run caught an outdated skill-version assertion and an interview-only preparation fixture; these were updated to the tightened contract. Independent exact-head review and deployment remain pending. The additive rollout is documented in [the runbook](../preparation-rollout.md).
