...
Production observation: a three-page PDF with the caption "when is this for?" returned the generic "Execution stopped after an error" message; bounded diagnostics showed the run failed 36 ms after its first model allocation with no provider diagnostics, while an image turn seconds later succeeded. Local reproduction with the same file: extraction succeeded (3 pages, 6,862 characters) and the fixed prompt reached about 36,700 characters before live runtime state, so the 48,000-character context guard threw once approvals, scopes and record inventories were added. Fix: the guard now drops history instead of failing the turn, traces `context.over_budget`, and keeps a 120,000-character hard limit; the inline excerpt is 4,000 characters. The exact production fixed size was not captured because the failure predates the new trace; the mechanism is confirmed by the timing signature and local measurement, not by a retained production prompt.
