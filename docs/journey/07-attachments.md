...
Issue #27 phase 2 moved image reading behind the specialist boundary. The coordinator no longer receives image bytes; `media_delegate` processes current-turn attachments and stored documents and returns compact referenced facts. See [journey 12](12-media-specialist.md) and [media processing](../media-specialist.md).

## Incident — 13 September 2026: PDF turn failed before the model call

Production observation: a three-page PDF with the caption "when is this for?" returned the generic "Execution stopped after an error" message; bounded diagnostics showed the run failed 36 ms after its first model allocation with no provider diagnostics, while an image turn seconds later succeeded. Local reproduction with the same file: extraction succeeded (3 pages, 6,862 characters) and the fixed prompt reached about 36,700 characters before live runtime state, leaving about 11,000 characters for approvals, scopes, skills and record inventories. The 48,000-character context guard throwing once live state was added is the most plausible mechanism; it is consistent with the timing and the local measurement, but the production fixed size and live state size were not captured, and a failure inside the pre-model state refresh would produce the same signature. Fix: the guard no longer fails the turn. The current message and its newest tool group are always sent; prior history and older in-turn groups share the unchanged allowance as before; `context.over_budget` records sizes; a 120,000-character hard limit refuses with recorded sizes. Independent review shaped this twice: the first fix dropped the current turn's own tool results, which would have caused repeated calls until the budget ran out, and the second retained the whole turn up to the hard limit, which would have widened the per-call ceiling for long research turns roughly threefold without a decision; the shipped floor design avoids both. Released in PR #34 as v0.3.4, deployed commit `1e44bdb179098e6e1b84b9c449023170bb907c55`, release run 34718925632 healthy. The original PDF question had not yet been re-sent when this was written.
