...
- **Release (verified 2026-09-23)**: [PR #80](https://github.com/akhilvuputuri/chief-agent/pull/80) was independently reviewed (APPROVE on `8b20e42` and on final head `6df216e`, recorded on the PR) and squash-merged as `2f57b242254124b5d92242220b61eb6ad8715e2a` after required checks passed. [Release run 54](https://github.com/akhilvuputuri/chief-agent/actions/runs/35867529102) deployed that exact SHA and startup health passed. App-only; no migration. Startup health does not establish feature acceptance: provider activation in the host `.env` and a first live alert remain outstanding. Classifying per-symbol batch errors is a separate follow-up.
...
**Release and operator closure — 26 September:** [PR #93](https://github.com/akhilvuputuri/chief-agent/pull/93) passed independent GPT-6 Astra review at exact head `7665709f32e98d0de1592e9d9070358a39637ff9`; the reviewer ran 31 stock tests, 31 Calendar/runtime tests and an additional restart scenario. [Main checks](https://github.com/akhilvuputuri/chief-agent/actions/runs/36233526553) and [automatic release](https://github.com/akhilvuputuri/chief-agent/actions/runs/36233697974) passed at merge `d96cdb70dff85343efb57baac8988462f8120601`. The separately approved operator SQL (SHA256 `07fa654e973f04cb26541227b5d8dc928b98cd953831a6990e3c8c80d0d24d66`) also passed synthetic success/seven refusal cases. A production rollback dry run preserved the entire original call state, followed by a conditional one-call commit at 09:43 UTC. The original error and timestamp remained unchanged, reconciliation evidence was appended, and no unresolved owner calls remained. No Calendar action was replayed or live stock alert triggered. Live verification then found the distinct provider-MIC defect below; startup health did not establish full stock acceptance.

### Follow-up — 26 September 2026: provider segment identity versus market calendar

**Measured observation:** a bounded live symbol lookup succeeded with configured credentials, but its US Nasdaq result carried `XNGS`. The static calendar accepted operating MIC `XNAS` and rejected the segment. Direct tests had used `XNAS` fixtures, so they did not exercise the provider's actual listing identity. This was a second independent blocker after the runtime uncertainty repair.

**Diagnosis and change:** ISO's registry identifies `XNGS` (Global Select), `XNMS` (Global Market) and `XNCM` (Capital Market) as active US segments of `XNAS`. Accept those three in the existing US session calendar; retain the original MIC in storage, deduplication and provider quote requests. Do not rewrite the instrument to a generic market or accept every exchange with a Nasdaq-looking name. Unknown/non-US venues remain rejected. [Official MIC registry](https://www.iso20022.org/market-identifier-codes).

**Validation:** new tests compare segment sessions on a regular day, weekend, holiday and early close. A provider-shaped regression exercises ambiguous cross-country listings, explicit exchange choice, saved segment identity, quote requests with that exact MIC, a valid alert observation and another owner's isolation. All 33 focused stock tests pass with mocked prices. Candidate v0.3.26 still needs full checks, independent review, deployment and bounded live acceptance. This does not establish current-market quote freshness or an actual alert delivery.
