...
## Temporary headroom — 22 September 2026, candidate v0.3.22

The requested ceiling increase had not shipped with the unrelated Chief rebrand. A continued mailbox request at 01:02:25 SGT failed before the provider call: fixed context 67,570 characters, preceding exchange 36,526, latest group 15,674, no older current-turn work; final serialized size 123,286. This demonstrates why a new continuation could immediately fail even after previous results were compacted. No private source text was needed to diagnose it.

Raise the application hard text ceiling from 120,000 to 400,000 characters, while retaining compaction pressure at 120,000 and the normal optional-history target at 48,000. This provides temporary headroom without simply delaying all compaction or loading more optional history. These are character policies, not token-capacity claims. Longer requests can still cost more; unbounded accumulation and large fixed context remain follow-up [issue #77](https://github.com/akhilvuputuri/chief-agent/issues/77).

A synthetic continuation with a large preceding exchange and source-linked mailbox results now passes above the old ceiling; both results compact, current intent and source/account references survive, and original journal messages remain unchanged. Existing oversized/unrecoverable and escaped-wire fixtures now exercise the higher rejection ceiling. Twelve focused continuity tests pass. No model changes, dollar caps, data reset, provider calls or automatic resumption of the user's request are part of this patch. Independent review, full CI and exact production release verification are required; the associated PR/release records final evidence.

## Verified v0.3.22 release — 22 September 2026

[PR #79](https://github.com/akhilvuputuri/chief-agent/pull/79) merged as `c936d7d630831f6f0c4b27dd62c5a06141a50201` after independent GPT-6 Astra and Devin reviews and passing checks. The [v0.3.22 release](https://github.com/akhilvuputuri/chief-agent/releases/tag/v0.3.22) records 343 application and 10 tooling tests passing. The [release workflow](https://github.com/akhilvuputuri/chief-agent/actions/runs/35634079343) succeeded, and the exact-commit receipt confirms deployment and startup health. Direct running-container inspection confirmed `contextHardLimit=400000` and `contextBudget=48000`; a synthetic 133,396-character continuation passed with wire compaction and zero provider calls. This validates the installed packing mechanics, not completion of the user's original mailbox request. The cumulative-growth cause is still open in [issue #77 and its staged plan](../context-management.md).
