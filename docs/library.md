...
Settled by the first real link on 20 September 2026: the phone enters the code the assistant displays (Libby then shows "Your data is being synchronized to the device that was displaying the code you entered"), but the code poll never reports `fulfilled`; it keeps answering `retained` while the card arrives on the identity itself. The ceremony therefore checks the sync every fourth poll (each a counted read, about 15 per full attempt) and, when a card is present, links that bearer as it is: no clone call and no re-mint, so the only token known to carry the card is never discarded on an unexercised assumption. `/library link` also tries the existing identity's token first, so an attempt that timed out before this fix links without a new code.
