...
Behavior now: the current turn (the user message plus this turn's own tool calls and results) is always kept, so the model never loses the results it just requested; prior history receives whatever remains; a `context.over_budget` event records the measured fixed and current-turn sizes; the trailing state line tells the model that earlier conversation is unavailable this turn. Only a fixed part beyond a 120,000-character hard limit is refused, and that refusal now records its sizes in `model.failed` instead of failing silently. Non-provider failures in the model step also record a bounded error identity. The inline PDF excerpt stays at 6,000 characters; reducing it, or compacting tool descriptions, are the next levers if `context.over_budget` events appear regularly.
