...
`tests/media.test.ts` covers coordinator image isolation, child tool set and image parts, stored extraction and trace scrubbing, cache reuse across turns by content hash, blocked readings not being cached or stored, stale attachment and foreign source rejection, quote-before-read and out-of-assignment read enforcement, nested delegation denial, denied writes, a separately configured media model, and the incomplete fallback shape. Wrong-quote-after-read and kind-mismatch rejections are implemented and were confirmed by the reviewer's reproduction but are not in the committed suite. `tests/attachments.test.ts` and `tests/telegram.test.ts` cover intake and the persisted note. No paid model call was made; real-photo reading quality, provider vision support under price-first routing and actual cost per image remain to be observed in use.
