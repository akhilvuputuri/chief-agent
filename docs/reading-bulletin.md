...
2. `reading_settings(...)` sets `interests` (`[{topic, keywords?}]`, matched as whole phrases in headline, excerpt, feed categories and feed topics), optional `languages` (ISO 639 codes, normalized so `en`, `en-US` and `eng` match; checked against the feed's declared language; undeclared languages pass), `preferredDomains`, `excludedDomains`, `mutedTopics`, `deliveryTime` (HH:MM), `timezone` (IANA), `itemsPerEdition` (1–5, default 5), `discoverySlots` (0–2, default 1), `enabled` and `paused`. Lists replace the stored value. Enabling, and any later change while enabled, is refused until interests, time, timezone and at least one active feed exist.
