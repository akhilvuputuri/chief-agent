...
## Authenticated canvas rollout (v0.3.5 candidate)

This section supersedes the holding-only milestone above when the canvas PR is reviewed and released. `deploy/miniapp/Caddyfile` now proxies only `/miniapp/`, its two named assets and `/api/miniapp/*` to loopback port 3000. The root redirects to the static shell; all other paths remain 404. Authentication and every owner check are in the application, described in [canvases](canvases.md). Do not activate these proxy routes before the application has passed authentication/ownership checks.

This release includes additive `db/011_canvases.sql` and a Compose migration/environment change. Follow the reviewed operator path; do not bypass the automated migration guard:

1. Require exact-head independent Astra approval and passing CI. Check no runtime run is active and serialize with the existing `/var/lock/companion-release.lock`. Preserve the previous app image/config and holding Caddyfile. Do not cancel user tasks to deploy.
2. Apply only migration 011 with `ON_ERROR_STOP` to the existing Postgres database. It adds canvas tables/indexes; existing data and schedules stay intact. Reapplying it is safe. Do not run deferred migration 010 or any historical reset.
3. Install the exact reviewed `compose.yaml` and `db/011_canvases.sql` into the server release baseline. Set only `MINIAPP_ORIGIN=https://companion.188-166-246-143.sslip.io` in the server's private environment file without printing that file. This is a public setting; existing credentials stay untouched. Install the reviewed `scripts/cloud-release.py` as the root-owned restricted handler to expose aggregate canvas diagnostics; this does not expand deploy permissions.
4. Merge the approved SHA and observe the normal main checks/release. It now sees the reviewed additive baseline and can deploy normally. Subsequent app changes need no operator step. The feature is disabled in environments without MINIAPP_ORIGIN.
5. Once the exact application release is healthy, validate and atomically install the reviewed Caddyfile, then reload the Caddy service. Verify public static assets, HTTPS, unauthenticated API rejection, unknown-path denial and localhost health. Use only a synthetic signed identity in a local test fixture; do not forge production Telegram login data as a live-user substitute.
6. Set the bot's menu button to `web_app` with text Canvases and URL `<origin>/miniapp/` using the existing server-side bot credentials (never print them). `/canvases` and `/app` also return a button and work without BotFather configuration. This changes navigation only; it sends no user message or grants permissions.
7. Ask the owner to open the menu or `/canvases` from Telegram and verify their roles. A first real canvas can be requested conversationally. Report synthetic test results separately from real Telegram/model acceptance. Record deployed SHA, migration, Caddy configuration hash, diagnostics and menu verification in the PR/issue/release notes.

Rollback: restore the previous application image and holding-only Caddyfile; remove the Mini App menu button if the interface is unavailable. Keep the additive tables and all revisions. Preserve any newly saved canvases; do not run a down migration or delete records. App/image rollback does not restore historical database contents. Caddy and the root-owned diagnostic handler remain operator-managed, while ordinary application fixes retain the existing automatic GitHub release path.
