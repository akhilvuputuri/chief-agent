...
Released baseline: v0.3.8 at `d0e33365c7cec7b7cb1eb64c22de7c09d5d9a314`. Current candidate: v0.3.9, checkpoint steering on `feature/checkpoint-steering`. Ordinary input waits for an in-flight model/tool result, then joins the same unbound run; task-bound work pauses for handoff. Read [the steering guide](docs/checkpoint-steering.md) for ordered photo/PDF/voice preparation, pending/sent delivery projection, cancellation/reset boundaries and migration014. The candidate requires integrated checks, exact-head independent Astra approval and the baseline-pinned operator rollout before ordinary GitHub deployments can use the new schema. It is not yet a release claim.
