...
export const action = z.discriminatedUnion("operation", [
  ...
  z
    .object({
      operation: z.literal("memory_history"),
      key: z.string().regex(/^[a-z_]{1,50}$/),
    })
    .strict(),
  z
...
export interface AgentRequest {
  ...
  message: string;
  history: unknown[];
  memories: { key: string; value: string; revisionId?: string }[];
