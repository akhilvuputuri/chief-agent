...
export class GmailTools {
  ...
  private async access(user: string) {
    ...
    this.expires = Date.now() + Math.max(0, t.expires_in - 60) * 1000;
    return this.token;
  }
  async call(
    user: string,
    operation: "gmail_search" | "gmail_read",
    value: string,
    pageToken?: string,
  ) {
    ...
    if (operation === "gmail_search")
      return z
        ...
        .parse(data);
    const m = messageSchema.parse(data);
    const body = plainBody(m.payload);
    return {
      id: m.id,
      threadId: m.threadId,
      headers: m.payload?.headers?.filter((h) =>
