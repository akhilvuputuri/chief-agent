...
def reasoning_config():
    effort = os.environ.get("HERMES_REASONING_EFFORT", "medium")
    if effort not in ("low", "medium", "high"):
        raise ValueError("Invalid HERMES_REASONING_EFFORT")
    return {"enabled": True, "effort": effort}


def provider_routing():
    import math
    prices = {"prompt": float(os.environ.get("OPENROUTER_MAX_INPUT_PRICE", "2")),
              "completion": float(os.environ.get("OPENROUTER_MAX_OUTPUT_PRICE", "10"))}
    if any(not math.isfinite(x) or x <= 0 for x in prices.values()):
        raise ValueError("Invalid OpenRouter price limit")
    return {"sort": "price", "max_price": prices, "require_parameters": True}
...
def run_turn(data, factory=None):
    ...
    try:
        agent = factory(model=os.environ["HERMES_MODEL"], api_key=os.environ.get("OPENROUTER_API_KEY"),
                        ...
                        request_overrides={"extra_body": {"provider": provider_routing(), "reasoning": reasoning_config()}},
