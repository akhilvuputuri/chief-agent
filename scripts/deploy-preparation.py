#!/usr/bin/env python3
"""Reviewed one-time operator rollout for preparation evidence migration 015."""
import fcntl
import json
import pathlib
import re
...
BASE = '16cb37f28625823df1c35d41bc7f8844db4b09b2'
IMAGE = 'hermes-companion-gateway'
MIGRATION = '015_preparation_chain.sql'
COMPOSE_ANCHOR = b'        "/migrations/014_checkpoint_steering.sql",\n'
    ...
    with tarfile.open(path) as archive:
        ...
        for member in members:
            ...
            if (not relative.parts or relative.is_absolute() or '..' in relative.parts
                    ...
                    or relative.parts[0] in {'RELEASE', '.git', '.RELEASE-preparation'}
...
def validate_changes(stage):
    ...
    if new.keys() - old.keys() != {MIGRATION}:
        raise RuntimeError('Unexpected migration set; only 015 is permitted')
    ...
    if (stage / 'compose.yaml').read_bytes() != expected:
        raise RuntimeError('Unexpected Compose change; only the migration 015 entry is permitted')
...
def write_release(content):
    pending = LIVE / '.RELEASE-preparation'
    try:
...
def main():
    if len(sys.argv) != 3 or not re.fullmatch('[0-9a-f]{40}', sys.argv[2]):
        raise RuntimeError('Usage: deploy-preparation.py <git-archive.tar> <exact reviewed main SHA>')
    archive_path, sha = sys.argv[1:]
    with open(LOCK, 'w') as lock:
        fcntl.flock(lock, fcntl.LOCK_EX | fcntl.LOCK_NB)
        old_release = (LIVE / 'RELEASE').read_bytes()
        if old_release.decode().strip() != BASE:
            raise RuntimeError('Preparation rollout requires the exact documented baseline; reconcile newer releases first')
        with tempfile.TemporaryDirectory(prefix='companion-preparation-') as temporary:
            ...
            try:
                ...
                if sql('SELECT count(*) FROM runtime_migrations WHERE version=15') != '1':
                    raise RuntimeError('Migration 015 marker unavailable')
                ...
                raise RuntimeError('Candidate failed; previous application healthy; any committed additive migration 015 and preparation evidence retained') from None
            # The trusted /usr/local/sbin release command is unchanged and is not installed here.
            print(json.dumps({'deployed': sha, 'healthy': True, 'migration': 15}))
