...
spec = importlib.util.spec_from_file_location(
    'deploy_preparation', pathlib.Path(__file__).with_name('deploy-preparation.py'))
release = importlib.util.module_from_spec(spec)
spec.loader.exec_module(release)
ROOT = pathlib.Path(__file__).resolve().parent.parent
SHA = 'a' * 40
BASE_COMPOSE = ('services:\n  migrate:\n    command:\n      [\n'
                '        "-f",\n        "/migrations/014_checkpoint_steering.sql",\n'
                '      ]\n  gateway:\n    read_only: true\n')


class PreparationRolloutTests(unittest.TestCase):
    def scenario(self, *, running=0, pending=0, baseline=None, requested_sha=SHA,
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            original = {
                ...
                'db/014_checkpoint_steering.sql': '-- original steering migration\n',
            }
            ...
            commands, statements = [], []
            database = {'migration15': False, 'tasks': [('existing-task', [])]}
            ...
            def sql(statement):
                ...
                if statement == "SELECT count(*) FROM conversation_inputs WHERE state IN ('queued','running')":
                    return str(pending)
                if statement == 'SELECT count(*) FROM runtime_migrations WHERE version=15':
                    return str(int(database['migration15']))
                self.assertEqual(statement, candidate['db/' + release.MIGRATION])
                self.assertNotRegex(statement.upper(), r'\b(DELETE|DROP|TRUNCATE|UPDATE)\b')
                if migration_failure:
                    raise RuntimeError('simulated transactional migration failure')
                database['migration15'] = True
                return ''
            ...
            with patch.object(release, 'LIVE', live), \
                    ...
                    patch.object(sys, 'argv', ['deploy-preparation.py', str(archive_path), requested_sha]), \
            ...
            self.assertEqual(database['migration15'], expected_migration)
            ...
            if not expected_error:
                ...
                if stop in commands:
                    ...
                    self.assertFalse(database['migration15'])
    ...
    def test_only_exact_compose_migration_addition_is_allowed(self):
        correct = BASE_COMPOSE.replace(release.COMPOSE_ANCHOR.decode(),
                                       (release.COMPOSE_ANCHOR + release.COMPOSE_ADDITION).decode())
