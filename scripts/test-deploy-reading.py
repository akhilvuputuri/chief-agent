...
spec = importlib.util.spec_from_file_location(
    'deploy_reading', pathlib.Path(__file__).with_name('deploy-reading.py'))
...
class ReadingRolloutTests(unittest.TestCase):
    ...
    def scenario(self, *, running=0, pending=0, baseline=None, requested_sha=SHA,
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            original = {
                ...
                'db/018_watchlist.sql': '-- original watchlist migration\n',
            }
            ...
            candidate['compose.yaml'] = BASE_COMPOSE.replace(
                ...
                (release.COMPOSE_MIGRATION_ANCHOR + release.COMPOSE_MIGRATION_ADDITION).decode())
            ...
            commands, statements = [], []
            database = {'migration19': False, 'items': [('existing-item', [])]}
            ...
            def sql(statement):
                ...
                if statement == "SELECT count(*) FROM conversation_inputs WHERE state IN ('queued','running')":
                    return str(pending)
                if statement == 'SELECT count(*) FROM runtime_migrations WHERE version=19':
                    return str(int(database['migration19']))
                self.assertEqual(statement, candidate['db/' + release.MIGRATION])
                # Additive DDL only; foreign-key ON DELETE actions are still additive.
                scrubbed = re.sub(r'ON DELETE (CASCADE|SET NULL)', '', statement.upper())
            ...
            with patch.object(release, 'LIVE', live), \
                    ...
                    patch.object(sys, 'argv', ['deploy-reading.py', str(archive_path), requested_sha]), \
            ...
            self.assertEqual(database['migration19'], expected_migration)
            ...
            if not expected_error:
                ...
                if stop in commands:
                    ...
                    self.assertFalse(database['migration19'])
    ...
    def test_unexpected_database_changes_refuse_before_stop(self):
        cases = [({'db/001_initial.sql': '-- modified'}, 'Historical migration'),
                 ...
                 ({'db/020_unrelated.sql': '-- unrelated'}, 'Unexpected migration set'),
    ...
    def test_only_exact_compose_additions_are_allowed(self):
        ...
        for invalid in [BASE_COMPOSE, correct.replace('read_only: true', 'read_only: false'),
                        correct + '  unreviewed: {}\n',
                        correct.replace('TAVILY_API_KEY: ${TAVILY_API_KEY:-}\n', 'TAVILY_API_KEY: ${TAVILY_API_KEY:-}\n      UNREVIEWED: x\n'),
