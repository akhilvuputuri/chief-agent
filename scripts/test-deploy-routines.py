...
spec = importlib.util.spec_from_file_location(
    'deploy_routines', pathlib.Path(__file__).with_name('deploy-routines.py'))
...
class RoutinesRolloutTests(unittest.TestCase):
    def scenario(self, *, running=0, pending=0, baseline=None, requested_sha=SHA,
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            original = {
                ...
                'db/016_library.sql': '-- original steering migration\n',
            }
            ...
            commands, statements = [], []
            database = {'migration17': False, 'tasks': [('existing-task', [])]}
            ...
            def sql(statement):
                ...
                if statement == "SELECT count(*) FROM conversation_inputs WHERE state IN ('queued','running')":
                    return str(pending)
                if statement == 'SELECT count(*) FROM runtime_migrations WHERE version=17':
                    return str(int(database['migration17']))
            ...
            with patch.object(release, 'LIVE', live), \
                    ...
                    patch.object(sys, 'argv', ['deploy-routines.py', str(archive_path), requested_sha]), \
            ...
            self.assertEqual(database['migration17'], expected_migration)
            ...
            if not expected_error:
                ...
                if stop in commands:
                    ...
                    self.assertFalse(database['migration17'])
    ...
    def test_running_work_and_pending_routines_refuse_before_stop(self):
