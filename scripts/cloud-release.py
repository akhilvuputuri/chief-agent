...
def main():
    ...
    if command == 'diagnose':
        print(json.dumps({'release': (LIVE/'RELEASE').read_text().strip(), 'runs': query("SELECT id,state,stop_reason,model,used_models,used_tools,used_ms,started_at FROM runtime_runs ORDER BY started_at DESC LIMIT 15"), 'model_failures': query("SELECT run_id,created_at,data FROM events WHERE type='model.failed' ORDER BY created_at DESC LIMIT 15"), 'tools': query("SELECT operation,state,count(*)::int FROM runtime_calls WHERE started_at > now()-interval '24 hours' GROUP BY operation,state"), 'costs': query("SELECT run_id,count(*)::int requests,sum(actual_usd) reported_usd,count(*) FILTER(WHERE actual_usd IS NULL)::int unknown_charges FROM provider_charges WHERE created_at > now()-interval '24 hours' GROUP BY run_id"), 'research': query("SELECT e.run_id child_run_id,e.data->>'parentRunId' parent_run_id,r.state,r.stop_reason,r.used_models,r.used_tools,r.used_ms,r.started_at FROM events e JOIN runtime_runs r ON r.id=e.run_id AND r.user_id=e.user_id WHERE e.type='research.child_started' AND e.created_at > now()-interval '7 days' ORDER BY e.created_at DESC LIMIT 100"), 'telegram_delivery': query("SELECT data->>'kind' kind,count(*)::int replies,sum((data->>'messages')::int)::int messages,sum((data->>'legacyChunks')::int)::int legacy_chunks,sum((data->>'noticeMessages')::int)::int notice_messages,count(DISTINCT run_id)::int delivery_groups FROM events WHERE type='telegram.delivered' AND created_at>now()-interval '7 days' GROUP BY data->>'kind'"), 'telegram_views': query("SELECT type,count(*)::int events,round(avg((data->>'firstTapMs')::numeric)) first_tap_ms FROM events WHERE type IN ('telegram.view_opened','telegram.view_tapped','telegram.view_failed') AND created_at>now()-interval '7 days' GROUP BY type"), 'canvases': query("SELECT type,count(*)::int events,count(DISTINCT run_id)::int runs FROM events WHERE type IN ('canvas.revised','canvas.read','canvas.viewed','canvas.conflict') AND created_at>now()-interval '7 days' GROUP BY type"), 'storage': query("SELECT (SELECT count(*)::int FROM message_contents) AS unique_contents,(SELECT coalesce(sum(octet_length(payload::text)),0) FROM message_contents) AS unique_logical_bytes,(SELECT count(*)::int FROM run_messages) AS run_references,(SELECT count(*)::int FROM conversation_messages) AS conversation_references,(SELECT coalesce(sum(octet_length(data::text)),0) FROM events WHERE type='research.model_input') AS detailed_research_trace_bytes"), 'conversation_control': query("SELECT type,run_id,created_at,data FROM events WHERE type IN ('conversation.routed','context.selected','context.failed','context.omitted','context.over_budget','telegram.input_received','telegram.input_ready','conversation.delivery') AND created_at>now()-interval '7 days' ORDER BY id DESC LIMIT 100"), 'input_states': query("SELECT state,preparation,count(*)::int inputs,count(consumed_at)::int consumed,round(avg(EXTRACT(EPOCH FROM started_at-received_at)*1000)) average_queue_ms,round(avg(EXTRACT(EPOCH FROM consumed_at-received_at)*1000)) average_consume_ms FROM conversation_inputs WHERE received_at>now()-interval '7 days' GROUP BY state,preparation"), 'checkpoint_steering': query("SELECT type,count(*)::int events,count(DISTINCT run_id)::int runs,max(created_at) latest_at FROM events WHERE type IN ('conversation.inputs_consumed','input.checkpointed','conversation.task_handoff','conversation.inputs_parked','conversation.delivery_withheld','conversation.delivery_cancelled') AND created_at>now()-interval '7 days' GROUP BY type"), 'input_preparation': query("SELECT i.preparation,count(*)::int inputs,count(r.ready_at)::int ready,round(avg(EXTRACT(EPOCH FROM r.ready_at-i.received_at)*1000)) average_ready_ms FROM conversation_inputs i LEFT JOIN LATERAL (SELECT min(e.created_at) ready_at FROM events e WHERE e.user_id=i.user_id AND e.run_id=i.id AND e.type='telegram.input_ready') r ON true WHERE i.received_at>now()-interval '7 days' AND i.metadata->>'preparing'='true' GROUP BY i.preparation"), 'conversation_delivery_states': query("SELECT delivery_state,count(*)::int messages,count(DISTINCT run_id)::int runs FROM conversation_messages GROUP BY delivery_state"), 'journaled_not_dispatched': query("SELECT operation,state,count(*)::int calls FROM runtime_calls WHERE result->>'code'='NOT_DISPATCHED' AND started_at>now()-interval '7 days' GROUP BY operation,state"), 'jobs': query('SELECT status,count(*)::int FROM jobs GROUP BY status')}))
        return
