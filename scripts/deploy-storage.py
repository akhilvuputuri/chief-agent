#!/usr/bin/env python3
"""One-time, reviewed operator rollout for message storage 012. Not a cloud SSH capability."""
import fcntl
import json
import pathlib
import re
...
BASE = '20564401c5130f79e2f90a8c802c32298c6c502e'
...
def main():
    ...
    with open('/var/lock/companion-release.lock', 'w') as lock:
        ...
        if (LIVE/'RELEASE').read_text().strip() != BASE:
            raise RuntimeError('Initial storage rollout requires the documented baseline; reconcile newer releases first')
        with tempfile.TemporaryDirectory(prefix='companion-storage-') as tmp:
            ...
            if set(p.name for p in (stage/'db').glob('*.sql')) - set(p.name for p in (LIVE/'db').glob('*.sql')) != {'012_message_storage.sql'}:
            ...
            try:
                sql((stage/'db/012_message_storage.sql').read_text())
                ...
                compose('stop','gateway')
                # Rehydrate latest complete normalized records before starting the old application.
                if sql("SELECT count(*) FROM runtime_migrations WHERE version=12") == '1':
                    sql((stage/'scripts/restore-legacy-history.sql').read_text())
                ...
                raise RuntimeError('Candidate failed; legacy history restored and previous application healthy')
            ...
            print(json.dumps({'deployed':sha,'healthy':True,'migration':12}))
