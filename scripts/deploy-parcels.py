#!/usr/bin/env python3
"""Reviewed one-time operator rollout for delivery tracker migration 019."""
import fcntl
import json
import pathlib
import re
...
# b22b09d is the release verified deployed and healthy on 23 September 2026.
# 7297202 and 7b1cff9 change only journal, release-note and status documents above it;
# either may be the live release when this runs. Any other RELEASE must be reconciled
# and re-reviewed; never rewrite RELEASE to match.
BASE = 'b22b09d0abca93bc6551468d0ad50ca68551f102'
BASES = {BASE, '7297202dc13f1e14de7164ef57d7c38d74be6cbc', '7b1cff9506ebb31159825ff166823b7377e21efa'}
IMAGE = 'hermes-companion-gateway'
MIGRATION = '019_parcels.sql'
COMPOSE_ANCHOR = b'        "/migrations/018_watchlist.sql",\n'
COMPOSE_ADDITION = b'        "-f",\n        "/migrations/019_parcels.sql",\n'
...
def main():
    if len(sys.argv) != 3 or not re.fullmatch('[0-9a-f]{40}', sys.argv[2]):
        raise RuntimeError('Usage: deploy-parcels.py <git-archive.tar> <exact reviewed main SHA>')
    archive_path, sha = sys.argv[1:]
    with open(LOCK, 'w') as lock:
        fcntl.flock(lock, fcntl.LOCK_EX | fcntl.LOCK_NB)
        old_release = (LIVE / 'RELEASE').read_bytes()
        if old_release.decode().strip() not in BASES:
            raise RuntimeError('Delivery tracker rollout requires the exact documented baseline; reconcile newer releases first')
        with tempfile.TemporaryDirectory(prefix='companion-parcels-') as temporary:
            ...
            if sql("SELECT count(*) FROM conversation_inputs WHERE state IN ('queued','running')") != '0':
                raise RuntimeError('Pending conversation input; retry after scheduled work and execution finish')
            ...
            try:
                ...
                raise RuntimeError('Candidate failed; previous application healthy; any committed additive migration 019 and saved parcel retained') from None
