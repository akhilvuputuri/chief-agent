#!/usr/bin/env python3
"""Reviewed operator rollout for conversation control 013. Not a cloud SSH capability."""
import fcntl
import json
import pathlib
import re
...
BASE = '099e7478ff5d33bb240740406f822e3a01e7a580'
...
def main():
    ...
    with open('/var/lock/companion-release.lock', 'w') as lock:
        ...
        if (LIVE/'RELEASE').read_text().strip() != BASE:
            raise RuntimeError('Conversation control rollout requires the documented baseline; reconcile newer releases first')
        with tempfile.TemporaryDirectory(prefix='companion-conversation-') as tmp:
            ...
            if set(p.name for p in (stage/'db').glob('*.sql')) - set(p.name for p in (LIVE/'db').glob('*.sql')) != {'013_conversation_control.sql'}:
            ...
            try:
                sql((stage/'db/013_conversation_control.sql').read_text())
                ...
                compose('stop','gateway')
                # Migration013 is additive apart from relaxing task uniqueness. Preserve all
                # new records on rollback; never reconstruct or replay conversation/actions.
                ...
                raise RuntimeError('Candidate failed; previous application healthy; additive conversation records retained')
            ...
            print(json.dumps({'deployed':sha,'healthy':True,'migration':13}))
