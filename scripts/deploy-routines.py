#!/usr/bin/env python3
"""Reviewed one-time operator rollout for scheduled routine migration 017."""
import fcntl
import json
import pathlib
import re
...
def validate_changes(stage):
    ...
    if new.keys() - old.keys() != {MIGRATION}:
        raise RuntimeError('Unexpected migration set; only 017 is permitted')
    ...
    if (stage / 'compose.yaml').read_bytes() != expected:
        raise RuntimeError('Unexpected Compose change; only the migration 017 entry is permitted')
...
def main():
    if len(sys.argv) != 3 or not re.fullmatch('[0-9a-f]{40}', sys.argv[2]):
        raise RuntimeError('Usage: deploy-routines.py <git-archive.tar> <exact reviewed main SHA>')
    ...
    with open(LOCK, 'w') as lock:
        ...
        with tempfile.TemporaryDirectory(prefix='companion-routines-') as temporary:
            ...
            try:
                ...
                if sql('SELECT count(*) FROM runtime_migrations WHERE version=17') != '1':
                    raise RuntimeError('Migration 017 marker unavailable')
                ...
                raise RuntimeError('Candidate failed; previous application healthy; any committed additive migration 017 and scheduled routine retained') from None
            # The trusted /usr/local/sbin release command is unchanged and is not installed here.
            print(json.dumps({'deployed': sha, 'healthy': True, 'migration': 17}))
